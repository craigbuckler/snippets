...
if (owl && owl.Dom && owl.Event && !owl.Ajax) {
	...
	owl.Ajax = function(form, callback) {
		...
		this.Call = function(callback) {
			...
			if (!this.Status.Active) {
				...
				if (request) {
					...
					// fetch form values
					if (this.Form) owl.Each(this.Form.elements, function(e) { if (e.name && e.value) thisAjax.Argument(e.name, e.value); });

					// add call count
					if (owl.Ajax.Config.CallCount != "") this.Argument(owl.Ajax.Config.CallCount, owl.Ajax.CallCount++);
		...
		// start
		this.Hijax(form);
		if (owl.Ajax.Config.AjaxIdentifier != "") this.Argument(owl.Ajax.Config.AjaxIdentifier, 1);
	...
	owl.Ajax.prototype.Hijax = function(form) {
		if (form && form.nodeName && form.nodeName.toLowerCase() == "form") {
			...
			// add a submit event
			var thisAjax = this;
			new owl.Event(form, "submit", function (evt) {
				evt.StopHandlers();
